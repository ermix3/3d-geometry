...
function initGL(gl) {
  ...
  gl.matrixModelViewProjection = gl.matrixProjection
    .clone()
    .multiply(gl.matrixModelView);
}
...
function initGLSL(gl) {
  ...
  // load phong shader
  Smal.createShaderProgram(
    gl,
    "glsl/gles_phongTex.vert",
    ...
    gl.uniform3fv(
      program.uniform.lightAttenuations,
...
function frame() {
  ...
  if (!gl.model || !gl.model.vboVertex) return;
  ...
  gl.matrixModelView = gl.matrixView.clone().multiply(gl.matrixModel);
  gl.uniformMatrix4fv(
    gl.program.uniform.matrixModelView,
    false,
  ...
  gl.uniformMatrix4fv(
    gl.program.uniform.matrixNormal,
    false,
    gl.matrixNormal.m,
  );
  ...
  gl.matrixModelViewProjection = gl.matrixProjection
    ...
    .multiply(gl.matrixModelView);
  gl.uniformMatrix4fv(
    gl.program.uniform.matrixModelViewProjection,
    false,
  ...
  gl.vertexAttribPointer(
    gl.program.attribute.vertexPosition,
    3,
    gl.FLOAT,
    false,
    gl.model.stride,
  ...
  gl.vertexAttribPointer(
    gl.program.attribute.vertexNormal,
    3,
    gl.FLOAT,
    false,
    gl.model.stride,
  ...
  gl.vertexAttribPointer(
    gl.program.attribute.vertexTexCoord0,
    2,
    gl.FLOAT,
    false,
    gl.model.stride,
  ...
  if (gl.wireframeEnabled) {
    gl.uniform4fv(
      gl.program.uniform.lightColor,
      new Float32Array([0, 0, 0, 1]),
    );
    gl.drawElements(
      gl.LINE_STRIP,
      gl.model.getIndexCount(),
      gl.UNSIGNED_SHORT,
      0,
    );
  }
...
function handlePointerMove(pe) {
  ...
  if (index == 0) {
    ...
    gl.pointers[0].delta = new Smal.Vector2(
    ...
    // remember delta movement
    gl.pointers[1].delta = new Smal.Vector2(
  ...
  if (gl.pointers.length == 2) {
    ...
    let v = new Vector2(
      gl.pointers[1].x - gl.pointers[0].x,
    ...
    */
    {
      ...
      let deltaDistance = 0;
      if (gl.pointers.distance > 0) deltaDistance = dist - gl.pointers.distance;
...
function setupTexture(gl, texture, image, repeat, type) {
  let format = gl.RGBA;
  if (type == TextureType.OCCLUSIONMAP) format = gl.LUMINANCE;
