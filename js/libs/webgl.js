...
function logWebGL(gl) {
  ...
  log("     Color: (" + gl.getParameter(gl.RED_BITS) + ", " +
      gl.getParameter(gl.GREEN_BITS) + ", " + gl.getParameter(gl.BLUE_BITS) +
      ", " + gl.getParameter(gl.ALPHA_BITS) + ") bits");
...
function initGL(gl) {
  ...
  gl.matrixModelViewProjection =
      gl.matrixProjection.clone().multiply(gl.matrixModelView);
}
...
function initGLSL(gl) {
  ...
  // load phong shader
  Smal.createShaderProgram(gl, "glsl/gles_phongTex.vert",
                           "glsl/gles_phongTex.frag")
      .then(program => {
        ...
        gl.uniform3fv(program.uniform.lightAttenuations,
                      gl.light.getAttenuations());
        gl.uniform4fv(program.uniform.materialAmbient,
                      gl.material.getAmbient());
        gl.uniform4fv(program.uniform.materialDiffuse,
                      gl.material.getDiffuse());
        gl.uniform4fv(program.uniform.materialSpecular,
                      gl.material.getSpecular());
...
function frame() {
  ...
  gl.matrixModelView = gl.matrixView.clone().multiply(gl.matrixModel);
  gl.uniformMatrix4fv(gl.program.uniform.matrixModelView, false,
                      gl.matrixModelView.m);
  ...
  gl.uniformMatrix4fv(gl.program.uniform.matrixNormal, false,
                      gl.matrixNormal.m);

  // compute projection matrix
  gl.matrixModelViewProjection =
      gl.matrixProjection.clone().multiply(gl.matrixModelView);
  gl.uniformMatrix4fv(gl.program.uniform.matrixModelViewProjection, false,
                      gl.matrixModelViewProjection.m);
  ...
  gl.vertexAttribPointer(gl.program.attribute.vertexPosition, 3, gl.FLOAT,
                         false, gl.model.stride, 0);
  gl.vertexAttribPointer(gl.program.attribute.vertexNormal, 3, gl.FLOAT, false,
                         gl.model.stride, 12);
  gl.vertexAttribPointer(gl.program.attribute.vertexTexCoord0, 2, gl.FLOAT,
                         false, gl.model.stride, 24);
  ...
  if (gl.wireframeEnabled) {
    gl.uniform4fv(gl.program.uniform.lightColor,
                  new Float32Array([ 0, 0, 0, 1 ]));
    gl.drawElements(gl.LINE_STRIP, gl.model.getIndexCount(), gl.UNSIGNED_SHORT,
                    0);
  }
...
function handleResize() {
  ...
  gl.matrixProjection = Smal.Matrix4.makePerspective(
      FOV_V, width / height, gl.camera.zNear, gl.camera.zFar);
...
function handlePointerMove(pe) {
  ...
  if (index == 0) {
    ...
    gl.pointers[0].delta =
        new Smal.Vector2((pe.pageX - offset.x) - gl.pointers[0].x,
    ...
    // remember delta movement
    gl.pointers[1].delta =
        new Smal.Vector2((pe.pageX - offset.x) - gl.pointers[1].x,
  ...
  // if 1 down, perform rotate
    ...
    if (pe.pointerType == "mouse" && gl.pointers[0].button == 2) {
      ...
      angle.x = gl.camera.pointerAngle.x +
                (gl.pointers[0].y - gl.pointers.downs[0].y) * SCALE_ROTATE;
      angle.y = gl.camera.pointerAngle.y -
                (gl.pointers[0].x - gl.pointers.downs[0].x) * SCALE_ROTATE;
...
function copyPointer(pe, offset = {
  x : 0,
  y : 0
}) {
  // return pointer data
  return {
    id : pe.pointerId,
    x : pe.clientX - offset.x,
    y : pe.clientY - offset.y
  };
}
...
function loadTexture(gl, url, repeat, callback) {
  ...
  setupDefaultTexture(
      gl, texture); // temporarily use default image until it is loaded
...
function stopShiftCamera() { gl.camera.stopShift(); }

function startZoomCamera(d) { gl.camera.startForward(0.5 * d, 2); }
function stopZoomCamera() { gl.camera.stopForward(); }

///////////////////////////////////////////////////////////////////////////////
// toggle flags
///////////////////////////////////////////////////////////////////////////////
function toggleTexture(flag) { gl.textureEnabled = flag; }
function toggleSpin(flag) { gl.spin = flag; }

function enableTexture() { gl.textureEnabled = true; }
function disableTexture() { gl.textureEnabled = false; }
function enableSpin() { gl.spin = true; }
function disableSpin() { gl.spin = false; }
